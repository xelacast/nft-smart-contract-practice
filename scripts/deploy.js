...
async function main() {
  ...
  await contract.connect(owner).giveawayMintBatch([addr1.address, addr2.address, owner.address]);
  ...
  // await contract.connect(owner).giveawayMintBatch([addr2.address, addr1.address, owner.address]);
  ...
  console.log("Balance of token id 0: ", await contract.balanceOf(addr2.address, 0))
  ...
  (await contract.connect(addr1).cuteeExchange(1, 100001, 4));
